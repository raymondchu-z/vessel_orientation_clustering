...
def process(dir):
    ...
    if os.path.exists(savepath+"/model.pkl"):#删除原来的
    ...
    if(len(files) < N_CLUSTERS):#不够类别数目则取第一张
        ...
        outprint = "IMO:{:<10},file nember:{:<5}"
        outprint = outprint.format(dir, len(files))
        exit(0)
    ...
    outprint = "IMO:{:<10},file nember:{:<5},time_hog:{:<8.3f},time_cluster:{:<8.3f},time:{:<8.3f}"
    outprint = outprint.format(dir, len(files), time_hog, time_cluster, time_hog+time_cluster)
    print(outprint)
